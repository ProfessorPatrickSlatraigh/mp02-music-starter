...
def open_or_build_database():
    ...
    if os.path.exists(DB_PATH):
        # Branch 1: database file already exists
        conn = sqlite3.connect(DB_PATH)
        conn.execute("PRAGMA foreign_keys = ON;")
        print(f"  Opened existing database: {DB_PATH}")
        return conn
    else:
        # Branch 2: first run — build and seed in-memory, then backup to disk
        print(f"  First run — building and seeding database...")
        ...
        # Open the newly created database file
        conn = sqlite3.connect(DB_PATH)
        conn.execute("PRAGMA foreign_keys = ON;")
